...
def validate_json(output: str, board: chess.Board):
    """
...
def regenerate_json(error: Exception, json_str: str):
    ...
    if isinstance(error, json.JSONDecodeError):
        reprompt_message[
            'error'] = f'Invalid JSON response: """{json_str}""" is not a valid JSON object. Regenerate your response, providing your thoughts and move in the correct JSON format: {schema}.'
    elif isinstance(error, KeyError):
        reprompt_message[
            'error'] = f'Invalid JSON response: """{json_str}""" is missing the "move" key. Regenerate your response, providing the "move" key in the correct JSON format: {schema}.'
    else:
        raise error
...
def regenerate_move(error, move, board):
    ...
    if isinstance(error, chess.IllegalMoveError):
        reprompt_message[
            'error'] = f"Illegal move: '{move}'. Regenerate your response to my last prompt, but this time provide a single, legal move in SAN format. Here are the current legal moves in SAN you can make: '''{', '.join(legal_moves_san)}'''."
    elif isinstance(error, chess.AmbiguousMoveError):
        reprompt_message[
            'error'] = f"Ambiguous move: '{move}'. Regenerate your response to my last prompt, but this time using either long-SAN by specifying the file of the origin piece (i.e Nhg8 instead of Ng8), or UCI format (i.e f6g8). Legal UCI moves: '''{', '.join(legal_moves_uci)}'''."
    elif isinstance(error, chess.InvalidMoveError):
        reprompt_message[
            'error'] = f"Invalid move format: '{move}'. Regenerate your response to my last prompt, but this time provide a single, legal move using either SAN-format or UCI-format. Legal SAN moves: '''{', '.join(legal_moves_san)}'''."
    else:
        raise error
    return reprompt_message
